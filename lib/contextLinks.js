...
contextLinks.prototype.searchDocs = function(data, callback) {
	...
	var parameter = {
		query 		: data.query || this.DEFAULT.query || '*',
		projectId 	: data.projectId || this.DEFAULT.projectId,
		command 	: 'searchDocs'
	}, 	tmpValue;
	...
	function doCallback(err, response) {
		...
		if (typeof data.facet !== 'undefined') {
			...
			response.facets.fields = 
				...
				(response.facets.fields.field instanceof Array) ? 	response.facets.fields.field :
																	[response.facets.fields.field];
		}		
		callback(err, response);
	}
	
	if (typeof data.length === 'number' || typeof this.DEFAULT.length === 'number') {
		parameter.length = data.length || this.DEFAULT.length;
	}
...
contextLinks.prototype._object2attr = function(data) {
